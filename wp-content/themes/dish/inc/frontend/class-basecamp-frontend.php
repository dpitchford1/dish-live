...
final class Frontend {
	...
	/**
	 * Replace all default WordPress body classes with a single slug-based class.
	 *
	 * Runs at PHP_INT_MAX to discard every class WP (and other plugins) added
	 * before us. Returns only:
	 *  - page slug for standard pages  → "page--{slug}"
	 *  - "page--home"                  for the front page
	 *  - "page--blog"                  for the posts index
	 *  - CPT slug for singular CPTs    → "page--{post-type}--{slug}"
	 *  - "page--archive--{post-type}" for CPT archives
	 *  - "page--{taxonomy}--{slug}"   for taxonomy archives
	 *
	 * Additional classes can be appended via the 'basecamp_body_class' filter.
	 *
	 * Usage in templates: <?php Basecamp_Frontend::body_class(); ?>
	 *
	 * @return void
	 */
	public static function body_class(): void {
		$slug = '';
		...
		if ( is_front_page() ) {
			...
			if ( $post_type === 'page' ) {
				...
				$slug = 'page--' . $post_type . '--' . $post->post_name;
			}
		} elseif ( is_post_type_archive() ) {
			$slug = 'page--archive--' . get_post_type();
		} elseif ( is_tax() || is_category() || is_tag() ) {
			$term = get_queried_object();
			$slug = 'page--' . $term->taxonomy . '--' . $term->slug;
		} elseif ( is_search() ) {
			$slug = 'page--search';
		} elseif ( is_404() ) {
			$slug = 'page--404';
		...
		$out = $slug ? [ sanitize_html_class( $slug ) ] : [];

		// Allow extra classes via filter — passes the primary slug as context.
		$extra = apply_filters( 'basecamp_body_class', [], $slug );
		if ( ! empty( $extra ) ) {
			$out = array_merge( $out, array_map( 'sanitize_html_class', (array) $extra ) );
		}

		if ( ! empty( $out ) ) {
			echo 'class="' . implode( ' ', $out ) . '"';
		}
	}

	/**
	 * Remove <p> tags wrapping images generated by the editor.
	 *
	...
	public static function output_critical_css( $css_file_path, $transient_key = 'basecamp_critical_css' ) {
		...
		if ( false === $css || (int) get_transient( $transient_key . '_mtime' ) !== $file_mtime ) {
			...
			$css = ltrim( $css, "\xEF\xBB\xBF" ); // Strip UTF-8 BOM — breaks @font-face when CSS is inlined
	 ...
	 *
	 * Three-layer detection so parent items stay highlighted for child pages
	 * and CPT singles that are not themselves in the menu:
	 *
	 *  1. WP native active classes (current-menu-item, current_page_ancestor, etc.)
	 *  2. Explicit page-ancestor walk — catches child pages whose parent IS in
	 *     the menu but whose own URL was never added to it.
	 *  3. URL-prefix match — catches CPT singles, custom links, and any hierarchy
	 *     WP misses (e.g. /about/return-policy/ → /about/ menu item).
	 *
	 ...
	 * @return array
	 */
	public static function menu_selected_class( $atts, $item, $args, $depth ): array {
		$item_classes = is_array( $item->classes ) ? $item->classes : [];

		// 1. WP native active classes.
		$active_classes = [
			'current-menu-item',
		...
		// 2. Explicit ancestor walk for child pages not added to the menu.
		// Checks whether the queried page is a descendant of the page this
		// menu item links to.
		if ( ! $is_active && is_singular() && $item->object === 'page' ) {
			$queried = get_queried_object();
			if ( $queried instanceof \WP_Post ) {
				$ancestors = get_post_ancestors( $queried->ID );
		...
		// 3. URL-prefix matching — covers CPT singles and any edge-cases the
		// two checks above miss. Builds a trailing-slashed comparison so that
		// /about/ doesn't falsely match /about-us/.
		// The homepage is excluded to prevent it matching everything.
		if ( ! $is_active && ! empty( $item->url ) ) {
			...
			if ( $item_url !== $home_url && str_starts_with( $current_url, $item_url ) ) {
				$is_active = true;
			}
		}
